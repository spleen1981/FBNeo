...
int CShaderList::InitShaderList()
{
...
int CShaderList::FreeShaderList()
{
	m_vecAvailShaderList.clear();
...
HRESULT CShaderList::OnNotify( XUINotify *hObj, BOOL& bHandled )
{	
...
HRESULT CShaderList::OnRescanShaders( char *szPath,  BOOL& bHandled )
{ 
	...
	if (m_ShaderListData.empty())
	{
			...
			strcpy(szShaderName, "GAME:\\Shaders\\*");
...
HRESULT CShaderList::OnInit(XUIMessageInit *pInitData, BOOL& bHandled)
{
...
HRESULT CShaderList::OnGetItemCountAll(
...
HRESULT CShaderList::OnGetSourceDataText(
...
HRESULT CFilterOptions::OnInit( XUIMessageInit* pInitData, BOOL& bHandled )
{
	...
	GetChildById( L"XuiRotate", &m_Rotate);
	GetChildById( L"XuiMirroredVert", &m_Mirror);
	...
	{
		m_Rotate.SetCheck(true);
	}
	else
	{
		m_Rotate.SetCheck(false);
	}

	if (nVidRotationAdjust&2)
	{
		m_Mirror.SetCheck(true);
	}
	else
	{
       ...
       BOOL& bHandled )
{
	...
	else if ( hObjPressed == m_ShaderList )
    {
		char tmpShader[MAX_PATH];
		nIndex = m_ShaderList.GetCurSel();
				
		LPCWSTR shaderName = m_ShaderList.GetText(nIndex);
			 
		sprintf(tmpShader,"GAME:\\Shaders\\%S",shaderName);			 							
		 
		if( psEffect->LoadEffect(tmpShader) != S_OK)
		{
			const WCHAR * button_text = L"OK"; 		
			wchar_t msg[1024];
		...
		{
			strcpy(selectedShader,tmpShader);
			psEnabled = true;
		}
