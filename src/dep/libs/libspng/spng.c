...
static int read_chunk_bytes2(spng_ctx *ctx, void *out, uint32_t bytes)
{
    ...
    while(bytes)
    {
        ...
        if(!ctx->skip_crc) ctx->cur_actual_crc = crc32(ctx->cur_actual_crc, (const Bytef *)out, len);
...
static int spng__inflate_stream(spng_ctx *ctx, char **out, size_t *len, size_t extra, const void *start_buf, size_t start_len)
{
    ...
    if(start_buf != NULL && start_len)
    {
        ...
        stream->next_in = (const Bytef *)start_buf;
    }
    else
    {
        stream->avail_in = 0;
    ...
    while( !(ret = read_header(ctx)))
    {
        ...
        else /* Arbitrary-length chunk */
        {
            ...
            else if(!memcmp(chunk.type, type_splt, 4))
            {
                ...
                const unsigned char *keyword_nul = (const unsigned char *)memchr(data, 0, keyword_len);
                ...
                splt->entries = (spng_splt_entry *)spng__malloc(ctx, list_size);
            ...
            else /* Unknown chunk */
            {
                ...
                if(buf == NULL) return SPNG_EMEM;
                ctx->chunk_list = (spng_unknown_chunk *)buf;
...
static int write_chunks_before_idat(spng_ctx *ctx)
{
    ...
    if(ctx->stored.iccp)
    {
        ...
        Bytef *buf = (Bytef *)spng__malloc(ctx, dest_len);
        if(buf == NULL) return SPNG_EMEM;
    ...
    if(ctx->stored.text)
    {
        ...
        for(i=0; i < ctx->n_text; i++)
        {
            ...
            if(text->compression_flag)
            {
                ...
                compressed_text = (Bytef *)spng__malloc(ctx, dest_len);
...
static int write_idat_bytes(spng_ctx *ctx, const void *scanline, size_t len, int flush)
{
    ...
    zstream->next_in = (const Bytef *)scanline;
...
int spng_set_text(spng_ctx *ctx, struct spng_text *text, uint32_t n_text)
{
    ...
    struct spng_text2 *text_list = (spng_text2 *)spng__calloc(ctx, sizeof(struct spng_text2), n_text);
